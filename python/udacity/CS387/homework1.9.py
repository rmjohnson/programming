...
def xor_string(a,b):
    x = xor(a,b)
    return bits_to_string(convert_to_bits(x))

def get_m2(m1,c1,c2):
    k = xor(m1,c1)
    k = list(str(k))
    k = pad_bits(k,len(list(str(c2)))-len(k))
    k = ''.join(map(str,k))
    m2 = xor(c2,k)
    return bits_to_string(m2)
...
print get_m2(display_bits(string_to_bits(plaintext1)),str(C1),str(C2))
